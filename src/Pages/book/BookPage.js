...
import BookStoreContext from "../../components/context/bookStorContext";
...
import   CartContext  from '../../components/context/bookStorContext.js';
import BookStoreProvider from '../../components/context/BookStoreProvider';
...
const BookPage = () =>{
  ...
  const { id } = useParams();
  const book = books.find((b) => b.id === +id); //paresInt(id)=+id
  const [qty, setQty] = useState(1);
  if (!book) {
    return <></>;
  }
  ...
  <div className="book-content">
    ...
    <div className="book-content-info">
      ...
      <Rating rating={book.rating} reviews={book.reviews} />
      <div className="book-add-to-cart">
      <input className="book-add-to-cart-input"
              type="number"
              min="1"
              max="100"
              value={qty}
